...
//Document and Model are distinct classes in Mongoose. The Model class is a subclass of the Document class. When you use the
// Model constructor, you create a new document.
...
console.log(doc);
//In Mongoose, a "document" generally means an instance of a model. You should not have to create an instance of the 
...
const docs = await MyModel.findOne();

docs instanceof MyModel; // true
docs instanceof mongoose.Model; // true
docs instanceof mongoose.Document; // true
//Updating Using save()
//Mongoose documents track changes. You can modify a document using vanilla JavaScript assignments and Mongoose will convert
// it into MongoDB update operators.
...
//The save() function is generally the right way to update a document with Mongoose. With save(), you get full validation
//and middleware.

//For cases when save() isn't flexible enough, Mongoose lets you create your own MongoDB updates with casting, middleware,
// and limited validation.

// Update all documents in the `mymodels` collection
await MyModel.updateMany({}, { $set: { name: 'foo' } });
//Note that update(), updateMany(), findOneAndUpdate(), etc. do not execute save() middleware. If you need save middleware
// and full validation, first query for the document and then save() it.

//Validating
//Documents are casted and validated before they are saved. Mongoose first casts values to the specified type and then 
//validates them. Internally, Mongoose calls the document's validate() method before saving.
